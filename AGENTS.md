...
- 会话正文使用 `sessions/` 下的独立 JSONL 日志与原文对象；禁止继续向已迁移的 `deepdesk.json` 会话数组写正文。迁移、备份、归档与永久删除边界见 `docs/session-storage.md`。`seed-ui-session` 需在客户端退出后运行，测试使用临时 userData。
