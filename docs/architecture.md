...
文件位于 Electron `userData` 目录，带 `version` 字段便于未来迁移。长期记忆默认本地捕获、保存和检索，不联网、不上传第三方；启动时会回扫已有本地会话补录高置信记忆，敏感凭据不自动记录。真正发给模型的只有本次请求命中的格式化上下文。未选择 Agent 工作目录时使用系统用户主目录。API Key、MCP Token、环境变量、请求头、连接器令牌和回复令牌写盘前统一经 Electron `safeStorage` 加密，运行时内存快照保持明文供主进程使用；系统安全存储不可用时保持兼容并输出警告。详细行为见 [memory.md](./memory.md)。
