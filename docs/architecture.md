...
8. 主进程生成期间维护会话检查点；正常结束前用最终 AI 回复刷新上下文组成，结束（done / error / abort）时 flush 剩余内容、持久化会话、清理流状态
...
工具结果按展示层既有上限显示；大型原文保存为本地对象，模型上下文使用有界预览和可读回的引用。预算计入消息封装、工具名称与参数、当轮实际工具 schema，并预留输出空间；主进程同步估算组成给上下文面板。Max 模式把 Agent 单次输出预算由 8192 提升到最高 32768 tokens，同时按窗口的 25% 封顶，装配时预留相同空间。

接近窗口上限时，启发式压缩器优先保留目标、约束、关键决定、错误、路径及工具信息，并限制摘要预算；压缩提示不进入模型历史。每次请求前校验工具调用链：移除孤立结果，把缺少结果的 `tool_calls` 补为明确的未完成结果。异常历史也按修复后的结构保存，避免切换模型服务后因协议校验差异导致失败。

## 3. 进程职责

### Main

- `session-journal.ts`：每会话 JSONL 增量记录、快照、轻量目录与故障恢复；`session-objects.ts`：会话隔离的内容寻址大文本存储。
- `session-recovery.ts`：启动时校验会话数组、兼容缺失历史的旧会话，并标记中断状态；无法确认的数据不按空历史覆盖。
- `run-persistence.ts`：主进程聊天/Agent 检查点投影、退出 flush 与工具原文卸载/分段读回，不依赖 Renderer 最后一条事件到达。

- `store.ts`：AppStore 持有设置与会话领域对象；设置走「tmp + rename」原子替换，会话交给独立日志模块，迁移完成后不再整包序列化所有会话
...
- `mcp-install.ts`：会话内 MCP 安装预检；校验 stdio 命令与参数、生成不执行程序的短时安装候选，并提供审批展示数据
...
工具结果需要裁剪时先保存原文，再给模型预览和引用；`read_context` 只读回当前会话的原文或列出引用，规划模式同样可用。该工具不接受任意文件路径，返回内容保持工具数据的低信任级别。现阶段启动仍校验日志并装载完整会话对象，没有实现全量历史按需加载或向量记忆。
...
1. 用户可在“设置 → MCP”保存服务器配置，也可在会话中要求安装直接可连接的 HTTP MCP 地址或具有明确启动命令的 stdio MCP；渲染层不启动进程、不直连远程 MCP。
2. 会话安装先由主进程短暂检查服务器身份与工具清单，再生成与当前运行绑定的 10 分钟短时凭证；所有权限模式下都必须经用户确认，确认后才持久化并正式连接。
3. 本地服务器由主进程通过 stdio 启动；会话预检 stdio 配置时不会启动程序，只有用户确认安装后才会执行。远程服务器通过 MCP Streamable HTTP 连接，可附带 Bearer Token 或自定义请求头。
