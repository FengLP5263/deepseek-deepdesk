...
- 浏览器桥接测试在 WebSocket 升级边界仅允许带测试标记的模拟扩展连接，避免已安装的真实扩展扫描本地端口后接管测试会话。
...
- 每会话 JSONL 增量/快照/索引重建、旧格式迁移中断与删除记录、防止记忆重复回填、原文分段读取与跨会话边界、流式检查点和 Electron 重启续聊；测试只使用临时 userData
