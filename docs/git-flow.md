...
DeepDesk 使用 `main` 与 `develop` 两个常驻分支。日常开发集成在 `develop`，只有达到发布标准的版本才进入 `main` 并创建版本标签。
...
7. 切回 `develop` 继续下一版本开发。如果发布阶段在 `main` 产生了额外提交，必须先将这些提交同步回 `develop`。

## 标签规则

- 标签格式固定为 `v<package.json version>`，例如 `v0.16.0`。
