...
- 会话历史与设置分离保存在 Electron `userData`：每会话独立 JSONL 增量日志、轻量索引和定期快照，主进程在生成期间保存检查点；旧会话自动备份并迁移。后台运行中的会话显示低速旋转状态，完成后切换为亮绿色未读标记，点开会话后清除；当前打开的会话不重复提示。格式兼容性、恢复与逻辑删除边界见 [会话存储](docs/session-storage.md)。
