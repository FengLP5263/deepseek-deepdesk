...
- 对话框模型菜单会汇总所有已配置服务的模型，支持按服务名、模型名或模型 ID 即时搜索，并可用方向键、Enter 和 Esc 完成选择或关闭；同一会话入口可直接跨供应商切换，会话同时持久化供应商和模型选择。
...
- Agent 失败步骤提供原地重试；用户可先切换到可用模型，再从出错任务处恢复生成，较早上下文仍会保留。
- 会话历史与设置分离保存在 Electron `userData`：每会话独立 JSONL 增量日志、轻量索引和定期快照，主进程在生成期间保存检查点；旧会话自动备份并迁移。后台运行中的会话显示低速旋转状态，完成后切换为亮绿色未读标记，点开会话后清除；当前打开的会话不重复提示。格式兼容性、恢复与删除边界见 [会话存储](docs/session-storage.md)。
- 会话三点菜单提供“归档会话”，从日常列表和搜索结果中隐藏，但保留历史、模型选择与待发送消息。可在“设置 → 归档”恢复，或二次确认后永久删除本地会话文件；旧备份、独立长期记忆及微信／飞书端记录不随之删除。
- 支持点击侧边栏“搜索任务”或按 `Ctrl+K`（macOS 为 `⌘K`）跨本地任务与连接器会话搜索标题和对话内容，并用方向键与回车快速打开。
- 侧边栏和 `Ctrl/Cmd+K` 最近任务会把置顶会话放在最前，其余会话按最近更新时间排列；置顶、取消置顶与完整任务导出均收纳在会话三点菜单中，不增加列表常驻操作按钮。会话可导出为 Markdown 或 JSON，导出前由系统文件对话框确认保存位置，连接器内部回复令牌不会写入导出文件。
...
- 首屏不加载设置、技能广场、Markdown/GFM 和代码高亮等非必要模块；相关能力在首次访问页面或出现 AI 正文时按需载入。
- 界面字体支持多种本地字体方案，并可按住 `Ctrl` 滚动鼠标滚轮在 80%–150% 范围内等比缩放文字、图标、间距和弹窗；缩放比例会随本地设置持久化。
- 提供上下文用量与组成可视化，真实计入系统指令、记忆、消息、工具调用、工具返回、当轮工具定义和回复预留；Max 模式会同步展示其最高 32K 动态回复预算，AI 回复完成后立即刷新实际占用。接近模型窗口限制时自动压缩较早上下文，并在会话中展示压缩前后的估算用量。
- 大型工具结果保存为本地原文对象，模型接收有界预览和引用，并可通过只读 `read_context` 工具分段读回；不会因此扩大工作目录或跨会话访问权限。
- 生成期间的新消息以紧贴输入框的轻量卡片进入会话级待发送队列，可编辑、移除或立即发送。
- Agent 未发送草稿按会话保存在本机，切换页面或重启后可继续编辑；成功发送后自动清除，并限制为最近 30 条、每条最多 20,000 字符。
- “Enter 发送”偏好同时作用于普通聊天和 Agent；关闭后 Enter 换行，`Ctrl+Enter`（macOS 为 `⌘Enter`）发送。
...
- 工具卡片可展开查看结果，并可直接复制完整的本地结果文本；复制操作不会改变卡片展开状态。
...
- “设置 → MCP → 导入 JSON”支持选择本地 UTF-8 JSON 文件或粘贴内容；校验通过、预览确认后批量添加，默认不连接、不执行命令，同名配置不会被覆盖。格式与限制见 [MCP 支持](docs/mcp.md)。
...
- 技能广场展示精选技能与推荐技能，支持搜索和分类筛选内置技能及可复用任务模板，不展示 SkillHub 或套件入口；连接器统一从侧边栏进入。
...
如需接入 MCP 工具，可以在“设置 → MCP”中添加本地进程或远程服务并点击“保存并连接”；也可以在会话中提供可直接连接的 HTTP MCP 端点，或可信来源给出的 stdio 启动命令，并明确要求 DeepDesk 安装。HTTP 服务会先读取身份和工具清单；stdio 服务会先展示精确命令且不启动程序。两种方式都只有在用户确认后，才由 DeepDesk 主进程保存、连接并向 Agent 暴露工具。GitHub、npm、MCP Registry 或普通介绍页本身不能作为服务端点直接安装；涉及 Token、环境变量或自定义请求头时仍需在设置页安全配置。MCP 配置仅保存在当前系统账号的 Electron `userData` 中，请勿运行来源不可信的命令。
